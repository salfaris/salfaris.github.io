...
I have been using the term _playlist_ quite loosely throughout this article so let's make it concrete. A playlist $\mathcal{P}$ of length $n$ is simply a finite set of songs $\{ s_1, s_2, \ldots, s_{n-1}, s_T\}$. It does not assume any ordering, and our logical arguments do not assume ordering unless mentioned otherwise. This definition aligns with what we had before and so the result(s) remain. 

As per @prp-paths1, we know that $N(4) = 16$ and reaching the target song in less than or equal to $k \leq n-1$ steps is given by $\sum_{j=0}^{k} P(n-1, j)$. These makes no assumption on where we start the path. Suppose now you start at song $s_3$ in the playlist. How many paths can you take take to reach your target song $s_T$? Let's start enumerating all the possible paths to reach $s_T$, but this time we highlight how many steps it takes to reach $s_T$ from $s_3$.
...
Let $\mathcal{P}$ be a playlist of length $n$. Given a starting song $s \in \mathcal{P}$ and a target song $s_T \in \mathcal{P}$, the number of paths to reach $s_T$ in less than $t$ steps is given by:
...
paths which is on the order of 67. This is an exorbitantly huge number with 67 zeros. If we were to compute $$N_{\text{paths} \leq t \text{ steps}}/N(n)$, we get 7.7% which is not honestly not too bad. But still... an 8% chance of finding my favourite song is quite low. Moral of the story? Make your playlists small.

## What next?

Here are some key directions we can take to explore.

1. What if instead of a single target favourite song $s_T$, we have a set of target songs $\{s_{T_1}, s_{T_2}, ...\}$. How will the analysis differ when we start at any arbitrary song?
