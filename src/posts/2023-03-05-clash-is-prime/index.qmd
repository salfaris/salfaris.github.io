...
If $n = ab$ is a composite integer such that $0 < a \leqslant b$, then $a \leqslant \sqrt{n} \leqslant b$.
:::

> Note: in this setting, $a$ and $b$ are any positive real number.

::: {.proof}

If $n$ is a perfect square, then $a = b = \sqrt{n}$. So suppose $n$ is not a perfect square. Then $a < b$ and in this case, we can seek a contradiction. Suppose $a < \sqrt{n}$ but also $b < \sqrt{n}$, then $ab < n$ which is a contradiction. Similarly, if both $a$ and $b$ are $> \sqrt{n}$, then $ab > n$, also a contradiction. So it has to be that $a < \sqrt{n} < b$. $\blacksquare$
...
By Lemma @lem-sqrt_decomp, we know that if $n = kb$ with $0 < k \leqslant b$, then $k \leqslant \sqrt{n}$. And if this decomposition of $n$ is a decomposition of integers (i.e. $k$ and $b$ are integers), then $k < \sqrt{n}$ or equivalently, we have $k \leqslant \lfloor \sqrt{n} \rfloor$. If $k$ is prime, then we are done. Otherwise, there exists a prime $p$ that divides $k$ (hence, divides $n$) that would also satisfy $p \leqslant \lfloor \sqrt{n} \rfloor$ as desired. $\blacksquare$

:::

> Note: for a linear traversal check (like we are doing, starting from 3, 4, 5, ...), we know that we will encounter any prime divisor $p$ of any composite divisor $k$ of $n$ first. So the last part of the proof is not necessary for our need, but we put it there for completeness.
